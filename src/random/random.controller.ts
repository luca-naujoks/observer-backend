...
export class RandomController {
    ...
    @Get('animes')
    @ApiOperation({ summary: 'Get the requested amout of random anime from the db'})
    ...
    @Get('series')
    @ApiOperation({ summary: 'Get the requested amout of random series from the db'})
    @ApiQuery({ name: 'amount', type: Number, example: 5, description: 'Number of items to return' })
    @ApiOkResponse({
    ...
    @Get('local')
    @ApiOperation({ summary: 'Get random Local Content based of the amount requested'})
    @ApiQuery({ name: 'amount', type: Number, example: 5, description: 'Number of items to return' })
    @ApiQuery({ name: 'type', type: String, example: 'anime', description: 'Type of content to return' })
    @ApiOkResponse({
        description: 'Get x random Local Content',
    })
    getRandomLocalContent(@Query('amount') limit: number, @Query('type') type: string) {
